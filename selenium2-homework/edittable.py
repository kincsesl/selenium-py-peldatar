...
def táblátki():  # Kiírja a tábla akt. állapotát.
    ...
    táblázat = driver.find_element_by_xpath("/html/body/div/div/div[2]/table")
...
def két_utolsót_ellenőriz(lista):
    ...
    for i in range(sorsz - 2, sorsz):
        ...
        for j in range(1, oszlopsz):
            ...
            sorlista.append(driver.find_element_by_xpath(xp).get_attribute("value"))
...
for i in range(7, 9):
    for j in range(1, oszlopsz):
        ...
        driver.find_element_by_xpath(xp).send_keys(lista[i - 7][j - 1])
